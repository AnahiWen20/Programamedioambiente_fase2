...
public class Navigation extends javax.swing.JPanel {
    ...
    public void addItem(Icon icon) {
        item.add(new ModelNavigation(icon, item.size(), null));
        repaint();
    }

    private int getSelectedIndex(Point mouse) {
        for (ModelNavigation i : item) {
    ...
    public void setBounds(int i, int i1, int i2, int i3) {
        ...
        if (selectedIndex >= 0) {
            if (!animator.isRunning()) {
                ModelNavigation d = item.get(selectedIndex);
    ...
    private void runBeforeEvent() {
        for (EventNavigation event : events) {
    ...
    public void addEvent(EventNavigation event) {
